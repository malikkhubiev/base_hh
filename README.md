...
## 10) Моки, логи и диагностика

- LLM mock (`USE_MOCK_LLM=true`): запросы генерируются эвристикой.
- HH mock (`USE_MOCK_HH=true`): используется данные из `logs/`.
- При реальном HH поиске сохраняются:
...
## 13) Краткий FAQ для разработчиков

- **Почему 3 уровня, а не 1?**  
  Чтобы быстро переключаться между шириной охвата и точностью выдачи.
